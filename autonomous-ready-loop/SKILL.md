...
7. 如项目需要强约束，新增或改造机械检查，用于验证队列格式、checkpoint 新鲜度、verification record、invocation log 或停止条件 marker。
8. 首次运行用 `BoundedBatch` 小轮数确认行为稳定；确认后使用 `RunUntilBoundary` 自动推进到边界。
