...
- Status output：由通用 `harness-visualization` 生成 `.harness/status.md` 和 `.harness/status.json`，供人类和后续 agent 查看当前 layer、ready、task packet、runner 和 verification 状态。
...
业务项目不应复制可视化逻辑。业务项目只负责维护 queue、change packet、checkpoint、invocation log 和 verification 事实。
